...
import { FENETRES } from "@/lib/parcours";
...
// Dérivées de lib/parcours.js : la liste des fenêtres n'existe qu'à un seul
// endroit, partagé avec le site et l'application.
export const FENETRES_LIVRAISON = FENETRES.map((f) => ({ id: f.cle, fr: f.fr }));
...
export async function choisirIntervenant(service, iso, opts = {}) {
  ...
  // Préférences du patient (domicile) : l'intervenant FAVORI passe devant
  // s'il est libre ; sinon la préférence homme/femme filtre les candidats —
  // et si personne ne correspond, on n'annule pas la réservation : le
  // souhait reste visible sur la demande pour un ajustement par l'équipe.
  let candidats = res.libres;
  if (service === "domicile") {
    if (opts.favoriId) {
      const favori = candidats.find((r) => r.id === Number(opts.favoriId));
      if (favori) return favori;
    }
    if (opts.prefGenre) {
      const filtres = candidats.filter((r) => r.genre === opts.prefGenre);
  ...
  const charges = await prisma.demande.groupBy({
    by: [champ],
    where: { [champ]: { in: candidats.map((r) => r.id) }, date: { startsWith: jour }, statut: { in: STATUTS_BLOQUANTS } },
    _count: true,
  });
  const charge = Object.fromEntries(charges.map((c) => [c[champ], c._count]));
  return [...candidats].sort((a, b) => (charge[a.id] || 0) - (charge[b.id] || 0))[0];
}
