...
export function validerEtape(etape, d = {}) {
  ...
  if (etape === "creneau") {
    ...
    if (!estRecurrent(d.service, d.type) && !d.urgent) {
...
export function construireDemande(d = {}) {
  ...
  return {
    ...
    // « au plus tôt » est la valeur qu'attend le reste de la plateforme pour
    // une demande sans heure fixée — c'est elle que lit la régulation.
    date: d.urgent ? "au plus tôt" : livraison ? propre(d.jour, 10) : propre(d.iso, 16),
    sousMode: d.urgent ? "urgent" : livraison ? "fenetre" : "ponctuel",
    ...
    ...(transport && coord(d.departLat) !== undefined && coord(d.destLat) !== undefined
      ...
      : {}),
    // Préférence homme/femme à domicile : un souhait, pas une exigence. Si
    // personne ne correspond, la demande n'est pas refusée — le souhait reste
    // visible pour un ajustement par l'équipe.
    prefGenre: d.service === "domicile" && ["homme", "femme"].includes(d.prefGenre) ? d.prefGenre : undefined,
    codePromo: propre(d.codePromo, 30),
    packId: d.packId || undefined,
    pourPatient: propre(d.pourPatient, 30),
    // Le nom voyage avec la demande quand elle est faite pour quelqu'un
    // d'autre : c'est précisément là qu'une demande anonyme est ingérable
    // pour l'équipe, qui ne connaît que le numéro du compte réservant.
    nom: propre(d.pourPatientNom, 80),
...
// Étapes RÉELLEMENT parcourues pour cette demande.
//
// Une prise en charge au plus tôt ne choisit pas de créneau : c'est la
// régulation qui cale l'heure. Afficher quand même une étape « date et
// heure » ferait compter quatre étapes pour un parcours qui n'en a que
// trois — et laisserait attendre un choix qui ne viendra jamais.
export function etapesPour(d = {}) {
  return d.urgent ? ETAPES.filter((e) => e.cle !== "creneau") : ETAPES;
}

export function etapeSuivante(cle, d = {}) {
  const liste = etapesPour(d);
  const i = liste.findIndex((e) => e.cle === cle);
  return i >= 0 && i < liste.length - 1 ? liste[i + 1].cle : null;
}

export function etapePrecedente(cle, d = {}) {
  const liste = etapesPour(d);
  const i = liste.findIndex((e) => e.cle === cle);
  return i > 0 ? liste[i - 1].cle : null;
}

export function numeroEtape(cle, d = {}) {
  return etapesPour(d).findIndex((e) => e.cle === cle) + 1;
}
