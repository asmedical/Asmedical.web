...
export async function creerFacture({ compteId, demandeId, souscriptionId, lignes, echeanceJours = 7, creePar, service, notes, statut = "EMISE", remiseExtra = null }) {
  ...
  let { montant: remiseTotal, detail: remiseDetail } = calculerRemise(sousTotal, remises);
  // Remise supplémentaire (code promo) — le total ne devient jamais négatif.
  if (remiseExtra?.montant > 0) {
    remiseTotal = Math.min(sousTotal, remiseTotal + Math.round(remiseExtra.montant));
    remiseDetail = [remiseDetail, remiseExtra.detail].filter(Boolean).join(" · ");
  }
  const emission = auj();
...
export async function imputerPaiement(tx, factureId, montant) {
  ...
  // Facture d'ABONNEMENT réglée → la souscription s'active pour une période
  // (activation UNIQUEMENT à la réception du paiement, jamais avant).
  if (statut === "PAYEE" && f.souscriptionId) {
    const s = await tx.souscription.findUnique({ where: { id: f.souscriptionId }, include: { plan: true } });
    if (s && s.statut === "EN_ATTENTE_PAIEMENT") {
      const { finPeriode } = await import("@/lib/paiementReservation");
      const debut = new Date().toISOString().slice(0, 10);
      await tx.souscription.update({
        where: { id: s.id },
        data: { statut: "ACTIF", debut, fin: finPeriode(debut, s.plan?.frequence || "mensuel") },
      });
    }
  }
  return { paye, statut };
...
// Tarification KILOMÉTRIQUE (Google Maps) — codes ajoutés s'ils manquent,
// SANS toucher aux tarifs existants. Tous modifiables dans l'admin,
// versionnés comme le reste : AUCUN prix codé en dur ailleurs.
export const TARIFS_DISTANCE = [
  { categorie: "transport", code: "transport_prise_en_charge", libelle: "Prise en charge (départ)", montant: 500, service: "transport" },
  { categorie: "transport", code: "transport_km", libelle: "Prix par kilomètre", montant: 60, unite: "km", service: "transport" },
  { categorie: "supplement", code: "ferie", libelle: "Supplément jour férié", montant: 500 },
  { categorie: "supplement", code: "accompagnement", libelle: "Supplément accompagnement", montant: 600 },
  { categorie: "supplement", code: "fauteuil", libelle: "Supplément fauteuil roulant", montant: 400 },
  { categorie: "supplement", code: "oxygene", libelle: "Supplément oxygène", montant: 800 },
];

export async function completerTarifsDistance(auteur = "système") {
  const existants = await prisma.tarif.findMany({
    where: { code: { in: TARIFS_DISTANCE.map((t) => t.code) }, fin: null },
    select: { code: true },
  });
  const deja = new Set(existants.map((t) => t.code));
  const manquants = TARIFS_DISTANCE.filter((t) => !deja.has(t.code));
  if (manquants.length) {
    await prisma.tarif.createMany({
      data: manquants.map((t) => ({ ...t, debut: auj(), creePar: auteur })),
    });
  }
...
// Jour férié ? (liste MM-JJ modifiable dans les réglages admin)
export function estFerie(date, joursFeries) {
  const mmjj = String(date || "").slice(5, 10);
  if (!/^\d{2}-\d{2}$/.test(mmjj)) return false;
  return String(joursFeries || "").split(",").map((s) => s.trim()).includes(mmjj);
}

// Précisions saisies à la réservation (besoins cochés, aller-retour, accès…).
function detailsDe(demande) {
  try {
    const p = JSON.parse(demande.details || "{}");
...
// Besoins cochés à la réservation (clés brutes b_fauteuil, b_oxygene…).
function besoinsDe(demande) {
  const p = detailsDe(demande);
  return Array.isArray(p.besoinsCles) ? p.besoinsCles : [];
}
...
// week-end / férié / urgence / accompagnement / fauteuil / oxygène.
// TRANSPORT KILOMÉTRIQUE : si la demande porte une distance calculée par
// l'itinéraire serveur (Google Maps) ET que les tarifs « prise en charge »
// + « prix par kilomètre » existent, la course est tarifée à la distance —
// sinon, comportement historique inchangé (course au forfait).
export function construireLignes(demande, tarifs, { suffixe = "", joursFeries = "" } = {}) {
  ...
  const lignes = [];
  const priseEnCharge = tarifs.find((t) => t.code === "transport_prise_en_charge");
  const prixKm = tarifs.find((t) => t.code === "transport_km");
  const km = Number(demande.distanceKm);
  // Aller-retour : « typeTrajet » sert à la classe de véhicule (simple /
  // accompagne / medicalise), il ne pouvait donc pas porter aussi le sens du
  // trajet. Le parcours guidé l'écrit dans les précisions ; l'ancienne valeur
  // reste acceptée pour les demandes déjà en base.
  const allerRetour =
    demande.service === "transport" &&
    (demande.typeTrajet === "aller_retour" || detailsDe(demande).allerRetour === true);

  if (demande.service === "transport" && Number.isFinite(km) && km > 0 && priseEnCharge && prixKm) {
    // Course au kilomètre : prise en charge + distance (×2 si aller-retour).
    lignes.push({ libelle: `${priseEnCharge.libelle}${suffixe}`, quantite: 1, prixUnitaire: priseEnCharge.montant, tarifId: priseEnCharge.id });
    const kmFactures = Math.max(1, Math.ceil(km * (allerRetour ? 2 : 1)));
    lignes.push({
      libelle: `Distance ${km} km${allerRetour ? " (aller-retour)" : ""} × ${prixKm.montant} DZD/km`,
      quantite: kmFactures, prixUnitaire: prixKm.montant, tarifId: prixKm.id,
    });
  } else {
    const base =
  ...
  const supplement = (code, condition) => {
    const t = tarifs.find((x) => x.code === code);
    if (t && condition) lignes.push({ libelle: t.libelle, quantite: 1, prixUnitaire: t.montant, tarifId: t.id });
  };
  const besoins = besoinsDe(demande);
  supplement("nuit", estNuit(demande.date));
  supplement("weekend", estWeekend(demande.date));
  supplement("ferie", estFerie(demande.date, joursFeries));
  supplement("urgence", demande.prioritaire);
  supplement("accompagnement", demande.service === "transport" && demande.typeTrajet === "accompagne");
  supplement("fauteuil", besoins.includes("b_fauteuil"));
  supplement("oxygene", besoins.includes("b_oxygene"));
  return lignes;
}
...
export async function estimerPrestation({ service, date, dureeMin, typeTrajet, prioritaire, compteId, packId, distanceKm, details }) {
  // Pack forfaitaire : le prix affiché est le prix du pack, point final.
  if (packId) {
    const pack = await prisma.pack.findUnique({ where: { id: Number(packId) } });
    if (pack?.actif) {
      return {
        lignes: [{ libelle: `${pack.nom} (forfait)`, quantite: 1, prixUnitaire: pack.prix, montant: pack.prix }],
        sousTotal: pack.prix, remiseTotal: 0, remiseDetail: null, total: pack.prix, devise: DEVISE,
      };
    }
  }
  const [tarifs, reglage] = await Promise.all([
    tarifsEnVigueur({ service, date: date ? String(date).slice(0, 10) : undefined }),
    prisma.reglage.findUnique({ where: { id: 1 } }),
  ]);
  const lignes = construireLignes(
    { service, date, dureeMin, typeTrajet, prioritaire, distanceKm, details },
    tarifs,
    { joursFeries: reglage?.joursFeries || "" }
  );
...
export async function facturerDemande(demande, { auteur = "Facturation automatique" } = {}) {
  ...
  // Lignes : PACK forfaitaire (une ligne, prix tout compris) sinon tarifs
  // EN VIGUEUR (copiés, non rétroactifs) — mêmes règles que l'estimation.
  let lignes = null;
  if (demande.packId) {
    const pack = await prisma.pack.findUnique({ where: { id: demande.packId } });
    if (pack) {
      lignes = [{ libelle: `${pack.nom} (forfait) — prestation n°${demande.id}`, quantite: 1, prixUnitaire: pack.prix }];
    }
  }
  if (!lignes) {
    const tarifs = await tarifsEnVigueur({ service: demande.service });
    lignes = construireLignes(demande, tarifs, { suffixe: ` — prestation n°${demande.id}`, joursFeries: reglage?.joursFeries || "" });
  }
  if (!lignes) return { ignoree: "sans_tarif" };

  // Prestation COUVERTE par un abonnement (vérifié à la réservation) :
  // la facture est émise pour la traçabilité, avec une remise totale
  // « Inclus dans l'abonnement » — le client ne doit rien.
  let remiseExtra = null;
  let codeValide = null;
  if (demande.souscriptionId) {
    const sous = await prisma.souscription.findUnique({
      where: { id: demande.souscriptionId }, include: { plan: true },
    });
    if (sous) {
      const sousTotal = lignes.reduce((sm, l) => sm + (l.quantite || 1) * l.prixUnitaire, 0);
      remiseExtra = { montant: sousTotal, detail: `Inclus dans l'abonnement ${sous.plan?.nom || "ASM"}` };
    }
  }
  if (!remiseExtra && demande.codePromo) {
    try {
      const { validerCode, remiseDuCode } = await import("@/lib/offres");
      // Plusieurs bons séparés par « + » : le CUMUL n'est accepté que si
      // TOUS les bons sont marqués cumulables par l'administrateur —
      // sinon seul le premier bon valide s'applique.
      const codes = [...new Set(String(demande.codePromo).split("+").map((x) => x.trim()).filter(Boolean))].slice(0, 3);
      const valides = [];
      for (const cx of codes) {
        const v = await validerCode(cx, { service: demande.service, telephone: demande.telephone });
        if (v.ok) valides.push(v.code);
      }
      const retenus = valides.length > 1 && !valides.every((x) => x.cumulable) ? valides.slice(0, 1) : valides;
      if (retenus.length) {
        const sousTotal = lignes.reduce((sm, l) => sm + (l.quantite || 1) * l.prixUnitaire, 0);
        let restant = sousTotal;
        let total = 0;
        for (const code of retenus) {
          const m = remiseDuCode(code, restant);
          total += m;
          restant -= m;
        }
        if (total > 0) {
          codeValide = retenus;
          remiseExtra = { montant: total, detail: retenus.map((x) => `Code ${x.code}`).join(" + ") };
        }
      }
    } catch {}
  }
  ...
  const facture = await creerFacture({
    compteId: compte.id, demandeId: demande.id, souscriptionId: demande.souscriptionId || null, lignes,
  ...
  if (codeValide) {
    try {
      const { consommerCode } = await import("@/lib/offres");
      for (const code of codeValide) {
        await consommerCode(code, { telephone: demande.telephone, demandeId: demande.id });
      }
    } catch {}
  }

  // Prépaiements faits À LA RÉSERVATION (ticket d'agence, paiement en ligne,
  // simulation) : rattachés à la facture et imputés — jamais bloquant.
  try {
    const { appliquerPrepaiements } = await import("@/lib/paiementReservation");
    await appliquerPrepaiements(facture, demande.id);
  } catch {}

  // Journal + notification (interne + push) au payeur. Le message change
  // selon la situation réelle : rien à régler (couvert / déjà payé) ou dû.
  try {
    const finale = await prisma.facture.findUnique({ where: { id: facture.id } });
    const rienARegler = finale.statut === "PAYEE" || finale.total - finale.paye <= 0;
    await prisma.journal.create({
      data: { auteur, action: "finance.facture.auto", entite: "facture", entiteId: String(facture.id), detail: `${facture.numero} · ${facture.total} DZD · demande n°${demande.id}${rienARegler ? " · rien à régler" : ""}` },
    });
    await prisma.notification.create({
      data: {
        userId: compte.userId, type: "info",
        titre: rienARegler ? "Prestation réglée ✓" : "Votre facture est disponible",
        corps: rienARegler
          ? `Prestation n°${demande.id} terminée — rien à régler (${finale.remiseDetail || "déjà payée"}). Document ${finale.numero} disponible.`
          : `Prestation n°${demande.id} terminée — facture ${finale.numero} de ${finale.total} DZD, à régler avant le ${finale.echeance}.`,
        auteur: "ASM Finances", statut: "NON_LU",
      },
    });
    const { envoyerPush } = await import("@/lib/pushEnvoi");
    await envoyerPush(compte.userId, {
      titre: rienARegler ? "Prestation réglée ✓" : "Votre facture ASM est disponible",
      corps: rienARegler ? `${finale.numero} — rien à régler.` : `${finale.numero} · ${finale.total} DZD — payez en ligne ou en espèces.`,
...
export async function facturerMensuel(compte, mois, { auteur = "Facturation mensuelle" } = {}) {
  ...
  for (const d of demandes) {
    ...
    const regl = await prisma.reglage.findUnique({ where: { id: 1 } });
    let pour = construireLignes(d, await tarifsEnVigueur({ service: d.service, date: jour }), { suffixe, joursFeries: regl?.joursFeries || "" });
    // Aucun tarif ne couvrait cette date (grille créée après coup) :
    // repli sur les tarifs actuellement en vigueur.
    if (!pour) pour = construireLignes(d, await tarifsEnVigueur({ service: d.service }), { suffixe, joursFeries: regl?.joursFeries || "" });
