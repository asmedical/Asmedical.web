...
export async function facturerDemande(demande, { auteur = "Facturation automatique" } = {}) {
  ...
  // Code promo / parrainage : validé maintenant, consommé APRÈS la facture.
  let remiseExtra = null;
  let codeValide = null;
  if (demande.codePromo) {
    try {
      const { validerCode, remiseDuCode } = await import("@/lib/offres");
      const v = await validerCode(demande.codePromo, { service: demande.service, telephone: demande.telephone });
      if (v.ok) {
        const sousTotal = lignes.reduce((sm, l) => sm + (l.quantite || 1) * l.prixUnitaire, 0);
        const montant = remiseDuCode(v.code, sousTotal);
        if (montant > 0) {
          codeValide = v.code;
          remiseExtra = { montant, detail: `Code ${v.code.code}` };
        }
      }
    } catch {}
  }
  ...
  if (codeValide) {
    try {
      ...
      await consommerCode(codeValide, { telephone: demande.telephone, demandeId: demande.id });
    } catch {}
  }
