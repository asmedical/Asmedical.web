...
export async function connexionIdentifiant(identifiant, password) {
  ...
  // Téléphone : que des chiffres (avec + / 00 / 0, espaces, tirets, parenthèses)
  // et au moins 8 chiffres → connexion directe par numéro (Supabase auth).
  const chiffres = saisie.replace(/\D/g, "");
  const estTel = !saisie.includes("@") && /^[+\d][\d\s.\-()]*$/.test(saisie) && chiffres.length >= 8;
  if (estTel) {
    const phone = normaliserTel(saisie);
