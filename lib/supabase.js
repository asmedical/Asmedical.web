...
// Au retour, l'utilisateur revient sur /connexion?oauth=retour où la session
// est détectée puis aiguillée selon le rôle, comme pour le SMS.
...
// MODE TEST (temporaire, variable OTP_TEST_CODE côté serveur) : si l'envoi
// SMS échoue mais que le mode test est actif, on laisse passer à l'écran
// code — le code de test y sera accepté.
export async function envoyerCode(phone) {
  ...
  if (error) {
    try {
      ...
      if (d?.actif) return; // mode test : on continue sans SMS
    } catch {}
    throw error;
  }
}
...
// Si la vérification SMS échoue, on tente le code de TEST côté serveur
// (route désactivée dès que la variable OTP_TEST_CODE est retirée).
