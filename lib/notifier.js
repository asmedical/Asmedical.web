...
export async function notifierEquipe(demande) {
  return alerterEquipe(resumeDemande(demande), demande);
}

// Alerte libre vers l'équipe (Telegram + webhook) — même canaux que les
// nouvelles demandes, pour tout événement terrain (patient prêt, retard…).
export async function alerterEquipe(texte, demande = null) {
  const envois = [];
  ...
  if (webhook) {
    envois.push(poster(webhook, demande ? { text: texte, demande } : { text: texte }));
  }
