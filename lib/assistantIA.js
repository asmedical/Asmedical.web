...
export async function testIA() {
  ...
  try {
    ...
    if (!rep.ok) {
      return { ...info, ok: false, statut: rep.status, raison: String(data?.error?.message || "erreur inconnue").slice(0, 400) };
    }
...
function modele() {
  // Surchargable en prod. Par défaut un modèle Flash (rapide + économique).
  return process.env.ASM_ASSISTANT_MODEL || "gemini-2.5-flash";
}
