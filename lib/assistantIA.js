// Cerveau de l'assistant ASM — appel à Claude (IA) CÔTÉ SERVEUR uniquement.
// La clé API vit exclusivement dans une variable d'environnement Vercel
// (ANTHROPIC_API_KEY) — jamais dans le code, jamais envoyée au navigateur.
...
function modele() {
  // Surchargable en prod (ex. claude-haiku-4-5 pour réduire le coût).
  return process.env.ASM_ASSISTANT_MODEL || "claude-opus-4-8";
}
...
FORMAT DE RÉPONSE : réponds STRICTEMENT en JSON { "reponse": "...", "action": "..." }.
- "reponse" : ton message au client, dans SA langue.
- "action" : UNE valeur parmi ${ACTIONS_IA.join(", ")} (un bouton d'aide sous ta
  réponse), ou "aucune" si aucun bouton n'est utile. "appeler" pour joindre un humain.`;
...
// Appelle Claude et renvoie { reponse, action }. Lève une erreur si la clé
// manque, si l'appel échoue, ou si l'IA refuse — l'appelant retombe alors sur
// le moteur guidé (repondreAssistant).
export async function demanderIA({ message, historique = [], langue = "fr", contexte = "" }) {
  ...
  // On ne garde que les derniers échanges pour le contexte (coût + latence).
  const messages = [];
  ...
  if (data.stop_reason === "refusal") throw new Error("assistant_ia_refus");

  const bloc = (data.content || []).find((b) => b.type === "text");
  const parse = extraireJSON(bloc?.text);
