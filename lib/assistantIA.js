...
// Diagnostic sûr : indique SI la clé est lue et SI Gemini répond, en isolant le
// champ fautif éventuel — SANS jamais renvoyer la clé.
export async function testIA() {
  ...
  // On teste plusieurs configs pour repérer précisément ce que le modèle refuse.
  const variantes = {
    minimal: { maxOutputTokens: 20 },
    json_schema: { maxOutputTokens: 40, responseMimeType: "application/json", responseSchema: schema },
    thinking: { maxOutputTokens: 20, thinkingConfig: { thinkingBudget: 0 } },
    // Config réelle utilisée par l'assistant (doit être « ok »).
    reel: { temperature: 0.3, maxOutputTokens: 800, responseMimeType: "application/json", responseSchema: schema },
  };

  const essayer = async (generationConfig) => {
    try {
      ...
      if (!rep.ok) return { ok: false, statut: rep.status, raison: String(data?.error?.message || "?").slice(0, 250) };
      const cand = data.candidates?.[0];
      const txt = (cand?.content?.parts || []).map((p) => p.text || "").join("");
      return { ok: Boolean(txt), statut: 200, finish: cand?.finishReason };
    } catch (e) {
      return { ok: false, raison: "réseau : " + String(e?.message || e).slice(0, 150) };
    }
  };

  const tests = {};
  for (const [nom, cfg] of Object.entries(variantes)) tests[nom] = await essayer(cfg);
  return { ...info, ok: Boolean(tests.reel?.ok), tests };
}
...
export async function demanderIA({ message, historique = [], langue = "fr", contexte = "" }) {
  ...
  const corps = {
    ...
    generationConfig: {
      temperature: 0.3,
      maxOutputTokens: 800,
      // Réponse structurée garantie : { reponse, action }. (Pas de
      // thinkingConfig : son format diffère selon la génération du modèle.)
